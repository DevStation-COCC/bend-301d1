...
let doctorsAbove10 = (doctor) => doctor.number > 10;

// `.filter()` => returns a new array of values based on conditional return statement
let filteredDocs = doctors.filter(doctorsAbove10); // => Returns a new array of doctors; only those with a `number` value greater than 10
...
function filterByBeginDate(doctor) {
  return doctor.begin > 1996;
}
console.log(doctors.filter(filterByBeginDate));
console.log(filteredDocs);
